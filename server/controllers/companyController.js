...
const handleCompanyRegister = async (req, res) => {
    try {
        ...
        if (existing) throw new Error("Company already exists with given email/phone.");
        ...
        if (!otpSent.success) throw new Error("Failed to send verification OTP.");
        ...
        res.status(400).json({ message: "Unable to register company!", error: err.message || err });
    }
};
